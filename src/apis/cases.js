export function getCaseIntro() {
  return {
    key: 'cases',
    titleLarge: 'Cases',
    titleSmall: 'Opportunities I have had to positively impact people on a broad scale.',
    titleSmallClass: 'bump-up',
    classname: 'sample-portfolio',
  };
}

export function getCases() {
  return [
    {
      align: '1',
      id: 'tasties',
      link: 'https://www.tasties.ca/',
      github: '',
      title: 'Tasties Kitchen',
      classname: 'tasties-case bg-white',
      tags: ['Branding ', 'ecommerce'],
      bannerImg: '../img/tasties/tasties-cover-lg.png',
      roles: [
        {
          title: 'Designer & Front End Developer',
          skills: 'Adobe CC, SCSS, PHP, JS, Wordpress, Woocommerce',
          task: 'This project was split into two components. Part One: design of the company brand which included a logo, label, and stationary. Part Two: Design and develop a site for Tasties Kitchen\'s. Including the new brand development, their products and a portal for customers to pay and sign up for a monthly food subscription.',
          complete: 'Early 2017',
        },
      ],
      contents: [
        {
          align: '1',
          classname: '',
          heading: 'Part One: Branding',
          text: 'In regards to the logo the client requested natural, fresh colors and a design of a badge style logo. The feeling the brand should portay is organic, fresh, vibrant, healthy living, treat your body right, rustic/root forming.',
        },
        {
          align: '2',
          classname: 'tasties-type',
          image: '../img/tasties/tasties-type.png',
        },
        {
          align: '3',
          classname: 'tasties-colors',
          image: '../img/tasties/tasties-colors.png',
        },
        {
          align: '4',
          classname: 'tasties-logo image-center image-300',
          image: '../img/tasties/tasties-logo.png',
        },
        {
          align: '5',
          classname: 'tasties-stati image-shadow image-right',
          image: '../img/tasties/tasties-stati.png',
        },
        {
          align: '6',
          classname: 'tasties-jars image-shadow',
          image: '../img/tasties/tasties-jars.png',
          heading: 'Label usage on product.',
        },
        {
          align: '7',
          classname: 'tasties-web',
          text: 'Built on wordpress with Woocommerce and Stripe integration. The client requested the site have a home page that had a "landing page style" of design for the service they provide, include a way for users to create orders for the biweekly or monthly subscriptions, a page for the weekly menu, a mission page, a blog page, and a contact page.',
          heading: 'Part Two: Web Development',
        },
        {
          align: '8',
          classname: 'tasties-home image-shadow',
          image: '../img/tasties/tasties-home.png',
        },
        {
          align: '9',
          classname: 'tasties-products image-shadow',
          image: '../img/tasties/tasties-products.png',
        },
        {
          align: '10',
          classname: 'tasties-product-single image-shadow',
          image: '../img/tasties/tasties-product-single.png',
        },
      ],
    },
    {
      align: '2',
      link: '',
      github: '',
      id: 'probiotic',
      title: 'Probiotic MD',
      classname: 'probiotic',
      tags: ['Branding ', 'ecommerce'],
      bannerImg: '../img/probiotic/cover-1.png',
      roles: [
        {
          title: 'Designer & Front End Developer',
          skills: 'Adobe CC, SCSS, PHP, JS',
          task: 'This project was split into two components. Part One: design of the company brand which included a logo, label, and stationary. Part Two: Design and develop a sales funnel that had the ability to take payments. The criteria included: pages be compliant with ad networks like Facebook and Adwords. Pages need to showcase the product, be eye catching and optimized for page conversions.',
          complete: 'Early 2017',
        },
      ],
      contents: [
        {
          align: '1',
          classname: '',
          heading: 'Branding',
          text: '',
        },
        {
          align: '2',
          classname: 'probiotic-type image-invert',
          image: '../img/probiotic/type.png',
        },
        {
          align: '3',
          classname: 'probiotic-colors image-shadow-d',
          image: '../img/probiotic/colors.png',
        },
        {
          align: '4',
          classname: 'probiotic-logo image-shadow-d image-center image-500',
          image: '../img/probiotic/logo.png',
        },
        {
          align: '5',
          classname: 'probiotic-bottle image-shadow-d image-center image-500 text-center',
          image: '../img/probiotic/3btl.png',
          heading: 'Product Usage & Label Development',
        },
        {
          align: '6',
          classname: 'probiotic-webinfo',
          heading: 'Web Development',
          text: '',
        },
        {
          align: '7',
          classname: 'probiotic-vsl image-shadow-d',
          image: '../img/probiotic/vsl.png',
          text: 'VSL: Video Sales Landing Page',
        },
        {
          align: '8',
          classname: 'probiotic-presell image-shadow-d',
          image: '../img/probiotic/presell.png',
          text: 'Presell: A information page about a product.',
        },
        {
          align: '9',
          classname: 'probiotic-cart image-shadow-d',
          image: '../img/probiotic/cart.png',
          text: 'Cart',
        },
      ],
    },
    {
      align: '12',
      id: 'ui-samples',
      link: 'https://interfacewithdesign.com',
      github: '',
      title: 'IWD - Project Samples',
      classname: 'ui-samples',
      tags: ['UI ', 'UX ', 'Service'],
      bannerImg: '../img/interfacewithdesign-screen.png',
      roles: [
        {
          title: 'UI/UX Designer',
          skills: 'Sketch, Adobe CC',
          task: 'Interface With Design is one of my side projects I run that provides a service for creating high quality user interfaces for its customers on a subscription based scheme. For each interface IWD work on we are provided UX sketches, wireframes, brand colors, and screen descriptions for each project. Here are some product samples some of the work completed:',
          complete: '2017 - Current',
        },
      ],
      contents: [
        {
          align: '0',
          heading: 'Cryptobyte',
          text: 'A crypto currency exchange',
        },
        {
          align: '1',
          classname: 'image-shadow-d iphone-size crypto',
          image: '../img/ui-projects/crypto/cryptobyte-title.png',
          heading: '',
          text: '',
        },
        {
          align: '2',
          classname: 'image-shadow-d iphone-size crypto',
          image: '../img/ui-projects/crypto/cryptobyte-market.png',
          heading: '',
          text: '',
        },
        {
          align: '3',
          classname: 'image-shadow-d iphone-size crypto',
          image: '../img/ui-projects/crypto/cryptobyte-single.png',
          heading: '',
          text: '',
        },
        {
          align: '4',
          classname: 'image-shadow-d iphone-size crypto',
          image: '../img/ui-projects/crypto/cryptobyte-holdings.png',
          heading: '',
          text: '',
        },
        {
          align: '10',
          heading: 'Lexton Furniture',
          text: 'A online furniture store.',
        },
        {
          align: '11',
          classname: 'image-shadow-d iphone-size lexton',
          image: '../img/ui-projects/furniture/lexton-title.png',
          heading: '',
          text: '',
        },
        {
          align: '12',
          classname: 'image-shadow-d iphone-size lexton',
          image: '../img/ui-projects/furniture/lexton-home.png',
          heading: '',
          text: '',
        },
        {
          align: '13',
          classname: 'image-shadow-d iphone-size lexton',
          image: '../img/ui-projects/furniture/lexton-product.png',
          heading: '',
          text: '',
        },
        {
          align: '14',
          classname: 'image-shadow-d iphone-size lexton',
          image: '../img/ui-projects/furniture/lexton-cart.png',
          heading: '',
          text: '',
        },
        {
          align: '20',
          heading: 'Fire Message',
          text: '',
        },
        {
          align: '21',
          classname: 'image-shadow-d iphone-size fm',
          image: '../img/ui-projects/message/fm-title.png',
          heading: '',
          text: '',
        },
        {
          align: '22',
          classname: 'image-shadow-d iphone-size fm',
          image: '../img/ui-projects/message/fm-login.png',
          heading: '',
          text: '',
        },
        {
          align: '23',
          classname: 'image-shadow-d iphone-size fm',
          image: '../img/ui-projects/message/fm-message.png',
          heading: '',
          text: '',
        },
        {
          align: '24',
          classname: 'image-shadow-d iphone-size fm',
          image: '../img/ui-projects/message/fm-convo.png',
          heading: '',
          text: '',
        },
      ],
    },
    {
      align: '3',
      link: 'http://www.highmountainventures.com/',
      github: '',
      id: 'mountain',
      title: 'High Mountain Ventures',
      classname: 'hmv bg-white',
      tags: ['Branding ', 'Wed Design'],
      bannerImg: '../img/hmv/hmv-bg.jpg',
      roles: [
        {
          title: 'Front End Developer',
          skills: 'Adobe CC, SCSS, HTML, JS',
          task: 'To design and develop a new company website for High Mountain Ventures. The websites criteria included a service area map widget, a contact form, package list and a home page.',
          complete: 'Early 2016',
        },
      ],
      contents: [
        // {
        //   align: '1',
        //   classname: '',
        //   heading: 'Project Overview',
        //   text: ''
        // },
        {
          align: '2',
          classname: 'image-shadow',
          image: '../img/hmv/home-mac.png',
        },
        {
          align: '3',
          classname: 'image-shadow',
          image: '../img/hmv/ventures-home.png',
        },
        {
          align: '4',
          classname: 'image-shadow',
          image: '../img/hmv/ventures-blog.png',
        },
        {
          align: '5',
          classname: 'image-shadow',
          image: '../img/hmv/ventures-area.png',
        },
      ],
    },
    {
      align: '4',
      id: 'watchit',
      link: '',
      github: '',
      title: 'WatchIt',
      classname: 'watchit',
      tags: ['UI ', 'UX ', 'Concept'],
      bannerImg: '../img/rolex-bg.jpg',
      roles: [
        {
          title: 'UI/UX Designer',
          skills: 'Adobe CC',
          task: 'To design a application with a company brand trageting iPhone and Android devices. The market that the app would be targeting is the wrist watch community & enthusiasts. The criteria of the app was to include a way to make watch backgrounds, use the brand colors which include the base color of blue, and to have a logo with a typographic design.',
          complete: 'Late 2016',
        },
      ],
      contents: [
        {
          align: '1',
          classname: 'image-shadow-d',
          image: '../img/watchit-case.png',
          heading: '',
          text: '',
        },
      ],
    },
    {
      align: '5',
      id: 'logoArchive',
      link: '',
      github: '',
      title: 'Logo Archive',
      classname: 'logo-archive bg-white',
      tags: ['Branding ', 'Graphic ', 'Design'],
      bannerImg: '../img/logo-archive/logo-archive-bg.jpg',
      roles: [
        {
          title: 'Designer',
          skills: 'Adobe CC',
          task: 'Mutiple brands that I have created for a variety of clients & Projects.',
          complete: '2015 - Early 2017',
        },
      ],
      contents: [
        {
          align: '2',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/cac-logo.png',
          heading: 'Interior Design Co.',
          text: '',
        },
        {
          align: '6',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/tasties.png',
          heading: 'Food Service',
          text: '',
        },
        {
          align: '8',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/cca.png',
          heading: 'Information',
          text: '',
        },
        {
          align: '9',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/pro-logo.png',
          heading: 'CPG',
          text: '',
        },
        {
          align: '7',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/news-today.png',
          heading: 'Online News Website',
          text: '',
        },
        {
          align: '1',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/zoo-exhibit.png',
          heading: 'Zoo Exhibit',
          text: '',
        },
        {
          align: '3',
          classname: 'image-shadow image-500 image-center',
          image: '../img/logo-archive/g-coffee.png',
          heading: 'Street Coffee Cart Business',
          text: '',
        },
        // {
        //   align: '4',
        //   classname: 'image-shadow image-500 image-center',
        //   image: '../img/logo-archive/golden-rivers.jpg',
        //   heading: 'Craft Brewery',
        //   text: '',
        // },
        // {
        //   align: '5',
        //   classname: 'image-shadow image-500 image-center',
        //   image: '../img/logo-archive/peach-slice.png',
        //   heading: 'A Peach & Apple Orchard',
        // },
      ],
    },
    {
      align: '6',
      id: 'print',
      link: '',
      github: '',
      title: 'Print Work',
      classname: 'print-work bg-white',
      tags: ['print ', 'Graphic', ' Design'],
      bannerImg: '../img/screen-print-bg.jpg',
      roles: [
        {
          title: 'Graphic Designer',
          skills: 'Design, Print Pre-process',
          task: 'Mutiple pieces of print work that I have created for a variety of clients.',
          complete: '2014 - Early 2017',
        },
      ],
      contents: [
        {
          align: '1',
          classname: '',
          heading: 'Human Rights Activist Series',
          text: 'This series was created to help spread awarness of human rights in our society today. The approch that was taken in creating this poster was to use a very minimalistic theme. The design is suppose to be understood by all from the very iconic images used in the design.',
        },
        {
          align: '2',
          classname: 'image-shadow image-uli',
          image: '../img/Human-Rights/Human-Rights-Spread.jpg',
          heading: '',
          text: '',
        },
        {
          align: '3',
          classname: '',
          heading: 'Bauhaus Design Case Study',
          text: 'The case studies the main task was to research and create a two panel design incorporating a design style or era and type specimen poster. The posters display the beginnings of typographic design of the font “Futura”, its development and its relationship to the Bauhaus design era. The posters were created utilizing the design style of Bauhaus and the font “Futura” as the type specimen for the poster.',
        },
        {
          align: '4',
          classname: 'image-shadow image-uli',
          image: '../img/Bauhaus/Bauhaus-double.jpg',
          heading: '',
          text: '',
        },
        {
          align: '5',
          classname: 'image-shadow image-uli',
          heading: 'Audatious Album Design',
          image: '../img/Audatious/Audatious-spread.jpg',
          text: '',
        },
      ],
    },
  ];
}
